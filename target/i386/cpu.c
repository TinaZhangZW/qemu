...
static bool x86_cpu_should_hide_arch_capabilities(const X86CPU *cpu)
{
    ...
    return IS_AMD_CPU(env) ||
           (cpu->vendor_cpu_abi_compat && IS_HYGON_CPU(env));
}
 ...
 * Enable the corrected Hygon behavior only for machine types where the CPU
 * ABI compat gate is on.
 */
...
static void x86_cpu_realizefn(DeviceState *dev, Error **errp)
{
    ...
    if (cpu->ucode_rev == 0) {
         ...
         */
        if (IS_AMD_CPU(env) ||
            (IS_HYGON_CPU(env) && cpu->vendor_cpu_abi_compat)) {
...
static const Property x86_cpu_properties[] = {
    ...
    DEFINE_PROP_BOOL("x-vendor-cpu-abi-compat", X86CPU,
                     vendor_cpu_abi_compat, true),
