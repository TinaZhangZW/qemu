...
class MemAddrCheck(QemuSystemTest):
    ...
    def test_phybits_low_tcg_q35_71_hygon(self):
        """
        Same as q35-7.1 AMD case except that here we check that Dhyana
        follows the same AMD IOMMU HT reserved GPA range.
        """
        self.ensure_64bit_binary()
        self.set_machine('pc-q35-7.1')
    ...
    def test_phybits_ok_tcg_q35_70_hygon(self):
        """
        Same as q35-7.1 Dhyana case except that here we check that the
        q35-7.0 compatibility setting keeps the old memory layout.
        """
        self.ensure_64bit_binary()
        self.set_machine('pc-q35-7.0')
        self.vm.add_args('-S', '-cpu', 'Dhyana,phys-bits=40',
